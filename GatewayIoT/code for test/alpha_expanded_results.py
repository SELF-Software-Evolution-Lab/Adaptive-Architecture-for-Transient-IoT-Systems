...
alpha = redis_server.lrange("star_trek_alpha_data", 0, 70)
y_true = redis_server.lrange("true_expanded_data", 0, 70)
y_pred = redis_server.lrange("predicted_expanded_data", 0, 70)
file_output = open('alpha_expanded_metric.txt', 'w')  # added
...
for i in range(len(y_true)):
    print("input", alpha[i], "\nTrue", y_true[i], "\nPredicted", y_pred[i])
    print()
    file_output.write(alpha[i] + ";" + y_true[i] + ";" + y_pred[i] + "\n")  # added
