...
bet = redis_server.lrange("expanded_data_2", 0, 70)
y_true = redis_server.lrange("true_beta_data_2", 0, 70)
y_pred = redis_server.lrange("predicted_beta_data_2", 0, 70)
file_output = open('expanded_beta_metric.txt', 'w')  # added
file_output.write("input;True;Predicted\n")  # added

for n, i in enumerate(y_pred):
    if i in ["Insignia", "Planet"]:
...
for i in range(len(y_true)):
    print("input", bet[i], "\nTrue", y_true[i], "\nPredicted", y_pred[i])
    print()
    file_output.write(bet[i].replace("ñ", "n") + ";" + y_true[i] + ";" + y_pred[i] + "\n")  # added
